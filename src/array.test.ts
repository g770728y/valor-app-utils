...
describe("replaceById", () => {
  // 比如 树形数组操作最后一步, 返回新的数组
  // 这个数组里, 有新的item, 也有老item(但引用不等)
  // 为了维持引用相等, 所以替换之
  const arr0 = [{ id: 1 }, { id: 2 }, { id: 3 }];
  const arr1 = [{ id: 1 }, { id: 3 }, { id: 4 }];
  it("case0", () => {
    expect(arr1[0]).not.toBe(arr0[0]);

    const result = replaceById(arr1, arr0);
